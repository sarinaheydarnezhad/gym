...
type Page = 'dashboard' | 'students' | 'library' | 'settings' | 'profile'
...
function SettingsPage({ dark, toggleDark }: { dark: boolean; toggleDark: () => void }) {
  return <>
    ...
    <main className="page settings-page">
      ...
      <section className="panel settings-card coach-subscription"><div className="subscription-heading"><div><small>مدیریت اشتراک مربی</small><h2>پلن حرفه‌ای شاگردیتو</h2><p>پرداخت مربی مستقل از اشتراک‌گذاری برنامه با شاگرد است.</p></div><span className="current-plan">پلن فعلی</span></div><div className="subscription-options"><article><strong>اشتراک ماهانه</strong><b>۴۹۰٬۰۰۰ <small>تومان / ماه</small></b><p>هزینه ثابت برای مربی‌های با تعداد شاگرد بالا</p></article><article className="recommended"><i>پیشنهادی</i><strong>پرداخت به‌ازای شاگرد</strong><b>۴۹٬۰۰۰ <small>تومان / شاگرد</small></b><p>شروع کم‌ریسک و رشد هزینه متناسب با درآمد</p></article></div></section>
...
export default function App() {
  ...
  const [dark, setDark] = useState(() => localStorage.getItem('hamrah-theme') !== 'light')
